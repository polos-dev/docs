...
import { defineAgent, defineTool, WorkflowContext } from "@polos/sdk";
...
const getWeather = defineTool(
  ...
  async (ctx: WorkflowContext, input) => {
    // Call weather API
    const data = await weatherApi.get(input.city);
    return {
      city: input.city,
      temperature: data.temp,
      condition: data.conditions,
...
async function main() {
  ...
  // Agent automatically:
  // 1. Calls getWeather("New York")
  // 2. Calls getWeather("London")
  // 3. Generates comparison
  // 4. Returns final answer
}
...
```typescript TypeScript
// Anthropic extended thinking
const response1 = await anthropicAgent.run(
  client,
  "What's the weather in New York? How did it impact my day?",
  {
    thinking: {
...
// OpenAI reasoning
const response2 = await openaiAgent.run(
  client,
  "Analyze the weather patterns",
  {
    reasoning_effort: "high", // OpenAI o1 models
...
// Temperature and other common parameters
const response3 = await weatherAgent.run(
  client,
  "What's the weather?",
  {
    temperature: 0.7,
...
```typescript TypeScript
const response = await weatherAgent.run("What's the weather in SF?");
...
for (const toolResult of response.toolResults) {
  ...
  console.log(`Input: ${toolResult.result}`);
}
...
```
@polos @sales-agent What's the latest on Acme?
```

`@sales-agent` is the ID of the agent you want to trigger. The bot routes the message to the right agent and streams the response back to the same thread.

To set this up, create a Slack app, configure event subscriptions, and register the app with your orchestrator. See [Slack Integration](/agents/slack-integration) for the full setup guide.
...
Polos supports three execution environments:
...
The `exec` tool supports security modes to control which commands run without approval: `allow-always` (default for Docker), `allowlist` (pattern-based), and `approval-always` (default for local). See [Sandbox](/agents/sandbox) for the full configuration reference.
...
```typescript TypeScript
// Agent execution starts
const response = await weatherAgent.run(client, "Compare weather in 5 cities");

// Agent calls getWeather("New York") ✓ - cached
// Agent calls getWeather("London") ✓ - cached
// Worker crashes ❌

// On retry:
// Agent resumes from last completed step
// getWeather("New York") - returns cached result (no API call)
// getWeather("London") - returns cached result (no API call)
// Continues with remaining cities
```
</CodeGroup>
...
const customerSupportWorkflow = defineWorkflow(
  async (ctx: WorkflowContext, input: CustomerSupportInput) => {
    // Run agent to handle customer query
    const agentResponse = await ctx.step.agentInvokeAndWait(
      "weather_agent",
      weatherAgent.withInput(input.question)
    );

    // Log the interaction
    await ctx.step.run("log", logInteraction, agentResponse);

    // Send follow-up email
    await ctx.step.run(
      "send_email",
      sendEmail,
      { to: input.customerEmail, body: agentResponse.result }
    );
...
- **[Sandbox](/agents/sandbox)** - Isolated execution environments for code and shell commands
