...
response = await safety_agent.run("How do I reset my password?")
```
...
3. If any rule fails, execution stops and an error is returned
...
```python
import re
from polos import Agent, guardrail, GuardrailContext, GuardrailResult, WorkflowContext
