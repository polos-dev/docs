...
export const sendEmail = defineTool(
  ...
  async (_ctx, input) => {
    await emailService.send({ to: input.to, subject: input.subject, body: input.body });
    return { status: 'sent' };
  },
);
```
...
[Sandbox tools](/agents/sandbox) have additional approval mechanisms beyond the `approval` parameter:

- **Exec security** controls command execution via `security` mode (`"approval-always"`, `"allowlist"`, or `"allow-always"`). See [Exec Security](/guides/19-exec-security).
...
See [Sandbox Tools](/guides/18-sandbox-tools) and [Local Sandbox](/guides/21-local-sandbox) for details.
