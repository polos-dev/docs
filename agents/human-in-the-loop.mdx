...
async def approval_workflow(ctx: WorkflowContext, input: ApprovalWorkflowInput):
    ...
    approval_data = await ctx.step.suspend(
...
```python
import asyncio
from polos import workflow, WorkflowContext, events, resume
from datetime import datetime
...
# Start workflow
async def main():
    handle = await charge_customer_workflow.invoke(ChargeRequest(
