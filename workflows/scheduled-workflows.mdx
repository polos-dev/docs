...
asyncio.run(schedule_user_reminders())
```
...
async def setup_user_schedule(user_id: str, preferences: dict):
...
await setup_user_schedule("user_123", {"timezone": "America/New_York"})
await setup_user_schedule("user_456", {"timezone": "Europe/London"})
```
