...
async def main():
    # Run the agent
    response = await weather_agent.run("What's the weather in NYC?")
...
Use `agent.run()` for simple execution:
...
```python
result = await weather_agent.stream("What's the weather in Tokyo?")
...
# First message
response1 = await chat_agent.run("What's the weather in NYC?", conversation_id=conversation_id)

# Follow-up (agent remembers context)
response2 = await chat_agent.run("How about tomorrow?", conversation_id=conversation_id)
...
async def approval_workflow(ctx: WorkflowContext, input: dict):
    ...
    # Suspend waits for an event with event_type="resume" to be received on topic f"{step_key}/{execution_id}"
    # Resumes here when the decision is received
    if decision.approved:
        # Execute the approved plan
        result = await ctx.step.agent_invoke_and_wait("execute", executor_agent.with_input(plan))
        return result
    else:
        return None
```
