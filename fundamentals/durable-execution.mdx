...
const processOrder = defineWorkflow<OrderInput, void, { status: string }>(
  { id: 'process_order' },
  async (ctx, input) => {
    const order = await ctx.step.run('fetch_order', () => fetchOrder(input.orderId));
    await ctx.step.run('charge_payment', () => chargePayment(order));
    await ctx.step.run('send_confirmation', () => sendConfirmation(order));
    return { status: 'completed' };
  },
);
```
...
```typescript TypeScript
const badExample = defineWorkflow<Input, void, void>(
  { id: 'bad_example' },
  async (ctx, input) => {
    // ❌ BAD: Random number determines execution path
    if (Math.random() > 0.5) {
      const result = await ctx.step.run('expensive_analysis', () => runAnalysis(input.data));
    } else {
      const result = await ctx.step.run('quick_check', () => runCheck(input.data));
    }

    await ctx.step.run('send_result', () => sendEmail(result));
  },
);
```
...
```typescript TypeScript
const goodExample = defineWorkflow<Input, void, void>(
  { id: 'good_example' },
  async (ctx, input) => {
    // ✅ GOOD: Random number generated in a step
    const shouldAnalyze = await ctx.step.run('decide', () => Math.random() > 0.5);

    let result;
    if (shouldAnalyze) {
      result = await ctx.step.run('expensive_analysis', () => runAnalysis(input.data));
    } else {
      result = await ctx.step.run('quick_check', () => runCheck(input.data));
    }
...
```typescript TypeScript
const orderWorkflow = defineWorkflow<Input, void, void>(
  { id: 'order_workflow' },
  async (ctx, input) => {
    // Built-in helpers (automatically durable)
    const timestamp = await ctx.step.now('get_time');
    const orderId = await ctx.step.uuid('gen_id');
    const randomVal = await ctx.step.random('get_random');

    await ctx.step.run('create_order', () => createOrder(orderId, timestamp));
  },
);
```
...
We recommend using Pydantic models (Python) or Zod schemas (TypeScript) for step outputs.
...
- **Step outputs must be JSON serializable** - Use Pydantic models (Python) or Zod schemas (TypeScript) for complex types
